        ...
        for (const auto& node_links : out_links) {
            ...
            switch (common_type) {
                ...
                case dai::DatatypeEnum::ImgDetections:
                    _pub_t[name] =
                            std::thread{generate_pub_lambda<depthai_datatype_msgs::RawImgDetections>(nh, name, 10)};
                    break;
                case dai::DatatypeEnum::ImgFrame: {
                    _pub_t[name] = std::thread{generate_pub_lambda<depthai_datatype_msgs::RawImgFrame>(nh, name, 10)};
                ...
                case dai::DatatypeEnum::NNData:
                    _pub_t[name] = std::thread{generate_pub_lambda<depthai_datatype_msgs::RawNNData>(nh, name, 10)};
                    break;
                ...
                case dai::DatatypeEnum::Tracklets:
                    _pub_t[name] = std::thread{generate_pub_lambda<depthai_datatype_msgs::RawTracklets>(nh, name, 10)};
                    break;
                default:
                    break;
            }
